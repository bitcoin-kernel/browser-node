...
- **Full hints-based stateless IBD** — the memory ceiling is **gone** (act ⑫ / `tools/swiftsync-commit.mjs`:
  the full 14.1M-coin UTXO set commits with **32 bytes of state**), and SwiftSync is wired into the running
  node (act ⑪ + `node.html`). The deepest remaining piece is a complete stateless forward-validation over a
  large range / genesis→tip: generate a **SwiftSync hints file** (<100 MB, WebTorrent-distributed), stream
  blocks + prevout data, and validate holding only the accumulator + a bounded working set — a full IBD that
  never materializes the UTXO set. Large snapshots/hints go via **WebTorrent**, never git.
